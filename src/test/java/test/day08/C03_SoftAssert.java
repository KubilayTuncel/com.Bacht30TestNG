...
public class C03_SoftAssert {
    ...
    public void test1(){
        ...
        selected.selectByValue("EUR");
        // 9. soft assert kullanarak "Eurozone (Euro)" secildigini test edin
        String actualDAta= selected.getFirstSelectedOption().getText();
        String expectedValue="Eurozone (euro)";
        SoftAssert softAssert= new SoftAssert();
        softAssert.assertEquals(actualDAta,expectedValue,"secilen option Eurozone degil");
        List<WebElement> tumOptionlar = selected.getOptions();
        //option listesoi webelementlerden olusuyor
        //expected liste ise String, bunun icin opsiyon listesini String yapmaliyiz
        List<String> strOptions=new ArrayList<>();
        tumOptionlar.stream().forEach(t->strOptions.add(t.getText()));
        List<String> expectedOptionsList= Arrays.asList("Select One", "Australia (dollar)",
                 "Canada (dollar)","Switzerland (franc)","China (yuan)","Denmark (krone)","Eurozone (euro)","Great Britain (pound)",
                 "Hong Kong (dollar)","Japan (yen)","Mexico (peso)","Norway (krone)","New Zealand (dollar)",
                 "Sweden (krona)","Singapore (dollar)","Thailand (baht)");
        softAssert.assertEquals(strOptions,expectedOptionsList,"listeler uyusmuyor");
